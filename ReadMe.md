...
3. 手机浏览器打开该链接（或直接扫二维码）→ 选图/拍照（**支持多选，单次最多 9 张**，九宫格预览、可单张删除）→ 手机端用 canvas 逐张自动压缩（最大边 1600px、JPEG 质量 0.75）→ 逐张按序上传到中继服务器（服务端按设备维护 FIFO 队列，最多暂存 9 条）；
...
- **手机上传页（内联）**：打开即每 8s 心跳报活；长轮询 `/phone/recv`，收到图片全屏展示+「长按保存」提示、收到文本展示+「复制文本」；顶部加在线状态指示。
