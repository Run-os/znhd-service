...
- **常用语管理**：从远程 YAML 配置文件加载常用语，支持关键字搜索过滤，一键复制并填入 TinyMCE 编辑器
- **工作时间限定**：仅在工作时间段（上午 9:00-12:00，下午 13:30-18:00）内执行监控，非工作时间自动暂停
...
1. 点击"常用语"按钮打开抽屉，自动从远程加载 [`public/commonPhrases.yaml`](public/commonPhrases.yaml:1)
...
[`znhd.user.js`](znhd.user.js:32) 中的 `CONFIG` 对象：
...
const CONFIG = {
    ...
    WORKING_HOURS: {
        MORNING: { START: 9, END: 12 },      // 上午工作时间
        AFTERNOON: { START: 13.5, END: 18 }  // 下午工作时间（13.5 = 13:30）
    },
